...
program.command('add <registry> <url> [home]').description('Add one custom registry').action(onAdd)
...
program.command('set-scope <scopeName> <value>').description('Associating a scope with a registry').action(onSetScope)

program.command('del-scope <scopeName>').description('Remove a scope').action(onDelScope)
...
program
  ...
  .action(onSet)
program.command('rename <registryName> <newName>').description('Set custom registry name').action(onRename)

program.command('del <registry>').description('Delete one custom registry').action(onDel)
...
function onSetScope(scopeName, value) {
  ...
  config([scopeRegistryKey], { [scopeRegistryKey]: value })
    .then(function () {
    ...
    new Promise((resolve, reject) => {
      if (currentRegistry) {
        if (currentRegistry[FIELD_REPOSITORY]) {
          printMsg(['', '   current registry is a custom registry, publish to custom repository.', ''])
